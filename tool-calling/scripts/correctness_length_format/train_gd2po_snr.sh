...
BASE_MODEL="${1:?Usage: $0 <model_path> [tau] [model_family]}"
TAU="${2:-0.8}"
MODEL_FAMILY="${3:-}"
EXP_NAME="gd2po-snr-correctness_length_format-tau${TAU}"
...
if [[ -z "${MODEL_FAMILY}" ]]; then
    ...
    if [[ "${BASE_MODEL_LOWER}" == *"qwen"* ]]; then
        ...
        echo "Please specify model_family as the third argument: $0 <model_path> <tau> <qwen|llama>" >&2
        exit 1
    fi
fi

export EXPERIMENT_NAME="${MODEL_FAMILY}"
export WITHCORRECT=1 WITHFORMAT=1 WITHLENGTH=1
...
echo "── ${EXP_NAME} | model=${BASE_MODEL} | tau=${TAU} ──"

python3 -u -m verl.trainer.main_ppo \
    algorithm.adv_estimator=gdpo_filter_ratio_conflict_group_ratio \
    algorithm.gdpo_filter_ratio_conflict_threshold="${TAU}" \
    algorithm.gdpo_filter_ratio_conflict_eps=1e-8 \
