...
def compute_score(solution_str, ground_truth, step=0):
    ...
    answer = [ground_truth]
    if str(os.getenv("WITHCORRECT", 1)) == "1":
        print("WITHCORRECT is set to 1, so correct score is set!")
        correctness_score = customize_correctness_reward_tool(completions, answer, step, tool_max_possible, tool_min_possible)[0]
    else:
        correctness_score = 0
    if str(os.getenv("WITHFORMAT", 1)) == "1":
        print("WITHFORMAT is set to 1, so format score is set!")
