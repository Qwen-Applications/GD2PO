...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    useful_score = batch.batch["rm_scores_useful"].sum(-1) if "rm_scores_useful" in batch.batch else None
    harmless_score = batch.batch["rm_scores_harmless"].sum(-1) if "rm_scores_harmless" in batch.batch else None
    ...
    if use_critic:
        ...
        # GDPO_GAE produces multi-head values with shape (B, S, N).
        # Reduce to (B, S) by averaging across heads before masking.
        if values.dim() == 3:
    ...
    metrics = {
        ...
        # returns
        "critic/useful_score/mean": torch.mean(useful_score).detach().item(),
        "critic/harmless_score/mean": torch.mean(harmless_score).detach().item(),
        **(
            {
                # values
