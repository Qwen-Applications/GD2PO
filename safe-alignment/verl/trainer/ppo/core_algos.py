...
class AdvantageEstimator(str, Enum):
    ...
    GPG = "gpg"
    GDPO = "gdpo"
    GDPO_Filter_GroupRatio = 'gdpo_filter_group_ratio'
    GDPO_Filter_RatioConflict_GroupRatio = 'gdpo_filter_ratio_conflict_group_ratio'
...
def _groupwise_mean_std(scores_1d: torch.Tensor, index: np.ndarray, epsilon: float):
    """
    scores_1d: (bs,) tensor
    return: mean/std tensors aligned with each example i: id2mean[index[i]], id2std[index[i]]
    """
    # collect per group
    id2score = defaultdict(list)
    for i in range(scores_1d.shape[0]):
        id2score[index[i]].append(scores_1d[i])

    id2mean = {}
    id2std = {}
    for idx in id2score:
        vals = torch.stack(id2score[idx], dim=0)  # (m,)
        if vals.numel() == 1:
            id2mean[idx] = torch.tensor(0.0, device=scores_1d.device, dtype=scores_1d.dtype)
            id2std[idx] = torch.tensor(1.0, device=scores_1d.device, dtype=scores_1d.dtype)
        else:
            id2mean[idx] = vals.mean()
            # note: torch.std default unbiased=True; verl 用法看你原代码。
            # 你的示例里是 torch.std(torch.tensor([id2score[idx]])) 其实没完全等价。
            # 这里用 unbiased=False 更稳定（一致性你可再改）。
            id2std[idx] = vals.std(unbiased=False)

    # map back
    means = torch.empty_like(scores_1d)
    stds = torch.empty_like(scores_1d)
    for i in range(scores_1d.shape[0]):
        means[i] = id2mean[index[i]]
