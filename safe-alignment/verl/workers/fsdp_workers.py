...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        use_align_anything_reward_model = config.model.get("use_align_anything_reward_model", False)
        use_shm = config.model.get("use_shm", False)
        # download the checkpoint from hdfs
        local_path = copy_to_local(config.model.path, use_shm=use_shm)
        self._use_align_anything_reward_model = use_align_anything_reward_model
        self._use_align_anything_formatter = config.model.get("use_align_anything_formatter", False)
        self.rm_formatter = None
        ...
        if self.config.model.input_tokenizer is None:
            ...
            if use_align_anything_reward_model:
                self.tokenizer = AutoTokenizer.from_pretrained(
                    local_path,
                    trust_remote_code=config.model.get("trust_remote_code", False),
                )
                self.tokenizer.model_max_length = config.get("max_length", 1024)
                self.tokenizer.padding_side = "right"
                if self._use_align_anything_formatter:
                    from .align_anything.configs.format_model import ModelFormatter

                    self.rm_formatter = ModelFormatter(self.tokenizer)
            else:
                self.tokenizer = hf_tokenizer(
                    local_path,
                    trust_remote_code=config.model.get("trust_remote_code", False),
                    model_max_length=config.get("max_length", 1024),
                )
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if use_align_anything_reward_model:
                from .align_anything.models.model_registry import AnyModelForScore

                reward_module = AnyModelForScore.from_pretrained(
                    local_path,
                    modality=[],
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if hasattr(output, 'end_scores'):
                    reward_rmpad = output.end_scores.squeeze(-1)  
                elif hasattr(output, 'logits'):
                    reward_rmpad = output.logits  
                else:
                    raise RuntimeError(f"Unexpected output format from reward model: {type(output)}")
                ...
                rm_score = output.end_scores.squeeze(-1)  
            ...
            if rm_score.dim() == 1:
                ...
                raise RuntimeError(f"Unexpected rm_score shape: {rm_score.shape}")
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            if self._use_align_anything_formatter and self.rm_formatter is not None:
                prompt_with_chat_template = self.rm_formatter(chat, add_generation_prompt=False)
            else:
            ...
            model_inputs = target_tokenizer(prompt_with_chat_template, return_tensors="pt")
